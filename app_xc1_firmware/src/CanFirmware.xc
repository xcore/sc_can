...
void canTestRxTx(chanend controlChan, chanend rxChan, chanend txChan,
		chanend rateChan, chanend ledChan, int bitZero) {
	struct CanPacket txPacket;
	struct CanPacket rxPacket;
	//Every thread for canTestRxTx will initiate
	//new mail box each with 32 message objects.
	MSGMEMORY stmsgMemory;

	/* There should be provision for differentiating
	 * given message object based on its priority.
	 * This can done by adding new variable
	 * for each message object.
	 * Also there should be some structure for
	 * message object register set for each thread ie.
	 * for each mail box.
	 */
	...
	//stmsgObject[0].RTR = CAN_REMOTE_FRAME;
...
select		{
		...
		if (command == SEND_PACKET) {
			...
			/* To transmit CAN packet next time, new CAN packet
			 * is intialized using randomizePacket()
			 * Instead , new packet should be fetched from
			 * the queue of object messages with
			 *  pending request for tranmission
			 */
			randomizePacket(txPacket, bitZero);
			txPacketCount++;
...
void ledManager(chanend client0, chanend client1, chanend client2, port led0,
		port led1, port led2, port ledGreen, port ledRed) {
...
int main() {
	...
	par
	{
		on stdcore[0]: canPhyRxTx(rxChan_0, txChan_0, rateChan_0, canClk_0,
				canRx_0, canTx_0);
		on stdcore[0]:
		canPhyRxTx(rxChan_1, txChan_1, rateChan_1, canClk_2, canRx_2, canTx_2);
		on stdcore[0]:
		ledManager(ledChan_0, ledChan_1, ledChan_2, led0_0, led1_0, led2_0,
				ledGreen_0, ledRed_0);

		on stdcore[1]:
		canTestRxTx(controlChan_0, rxChan_0, txChan_0, rateChan_0, ledChan_0, 0);
		on stdcore[1]:
		canTestRxTx(controlChan_1, rxChan_1, txChan_1, rateChan_1, ledChan_1, 1);
